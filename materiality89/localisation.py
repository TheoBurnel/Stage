...
def processer_fichier_entree_sortie(csv_entree, csv_sortie):
    # Charger les données en ne sélectionnant que les colonnes pertinentes
    data = pd.read_csv(csv_entree)
    ...
    final_data.to_csv(csv_sortie, index=False)

    print(f"Les données filtrées avec les lignes non retenues ont été enregistrées dans : {csv_sortie}")

# Appeler la fonction pour traiter materiality_net.csv et produire localisation.csv
processer_fichier_entree_sortie("donnees/materiality_net.csv", "donnees/localisation.csv")

# Appeler la fonction pour traiter materiality_net2.csv et produire localisation_2.csv
processer_fichier_entree_sortie("donnees/materiality_net2.csv", "donnees/localisation_2.csv")
